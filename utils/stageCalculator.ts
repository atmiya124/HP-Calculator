 ...
 *  - 4' supports cap the two outer ends when there's only a single row
 *    (chainedBy4 === 1 — a 1D chain has no internal seam to brace), but
 *    once there's more than one row every column boundary (including
 *    internal ones) needs full-height bracing: (chainedBy8+1) x chainedBy4.
 *
...
export function calculateStage(
  ...
  const support4ft = chainedBy4 === 1 ? 2 : (chainedBy8 + 1) * chainedBy4;
